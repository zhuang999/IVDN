...
def cf_train_quadkey(batch, data_source, max_len, sampler, quadkey_processor, loc2quadkey, num_neg, args):
    src_seq, trg_seq, t_mat, g_mat, mat2t, user_truncated_segment_index = zip(*batch)
    t_mat_ = torch.stack(t_mat)
    g_mat_ = torch.stack(g_mat)
    mat2t_ = torch.stack(mat2t)
    user_truncated_segment_index_ = torch.tensor(user_truncated_segment_index)
    src_locs_, src_users_, src_quadkeys_, src_times_ = [], [], [], []
    data_size = []
    for e in src_seq:
        u_, l_, q_, t_, _ = zip(*e)
        
        data_size.append(len(u_))
        src_users_.append(torch.tensor(u_))
    ...
    src_users_ = fix_length(src_users_, 1, max_len, 'train src seq')
    ...
    for i, seq in enumerate(trg_seq):
        pos = torch.tensor([[e[1]] for e in seq], dtype=torch.long)
    ...
    return src_locs_, src_users_, user_truncated_segment_index_, src_quadkeys_, src_times_, t_mat_, g_mat_, mat2t_, trg_locs_, trg_quadkeys_, data_size

def cf_eval_quadkey(batch, data_source, max_len, sampler, quadkey_processor, loc2quadkey, num_neg, args):
    src_seq, trg_seq, t_mat, g_mat, mat2t = zip(*batch)
    t_mat_ = torch.stack(t_mat)
    g_mat_ = torch.stack(g_mat)
    mat2t_ = torch.stack(mat2t)
    src_locs_, src_users_, src_quadkeys_, src_times_ = [], [], [], []
    ...
    src_users_ = fix_length(src_users_, 1, max_len, 'eval src seq')
    ...
    return src_locs_, src_users_, src_quadkeys_, src_times_, t_mat_, g_mat_, mat2t_, trg_locs_, trg_quadkeys_, data_size